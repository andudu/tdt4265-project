...
for edgePoint = 1:1:numEdgePoints
    ...
    for vectorNum = 1:1:pointCounter(binAngle)
        ...
        if xVote > 0 && xVote < imageSize(1) && yVote > 0 && ...
                yVote < imageSize(2)
            houghSpace(xVote, yVote) = houghSpace(xVote, yVote) + 1;
...
[x, y] = findMaxArea(houghSpace); % Note that we overwrite x and y.
...
function [x, y] = findMaxArea(houghSpace)

[rows, cols] = size(houghSpace);
maxIntensity = 0;
x = 0;
y = 0;
        ...
        for gridRow = 1:3
            for gridCol = 1:3
                accumulator = accumulator + ...
                    houghSpace(row + gridRow, col + gridCol);
